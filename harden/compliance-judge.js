...
Return ONLY valid JSON (no markdown):
{
  ...
  try {
    ...
    throw ApplicationFailure.create({
      message: `[BUILD-011] Failed to parse compliance evaluation: ${e.message}`,
      nonRetryable: false
    });
  }
  ...
  // FAIL — surface revision packages for escalation
  const revisionSummary = (compliance.revision_packages || [])
    .map(p => `${p.agent} [${p.priority}]:\n${p.revisions.map(r => `  - ${r}`).join('\n')}`)
    .join('\n\n');

  throw ApplicationFailure.create({
    message: `[BUILD-011] COMPLIANCE FAILED: ${score}% (threshold: ${COMPLIANCE_THRESHOLD * 100}%)\n\nRequired revisions:\n${revisionSummary}\n\nSummary: ${compliance.summary}`,
