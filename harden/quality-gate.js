...
Return ONLY valid JSON (no markdown):
{
  ...
  // Hard block only on blocking severity issues or explicit rejection
  const blockingIssues = (review.issues || []).filter(i => i.severity === 'blocking');

  if (review.approval_recommendation === 'reject' || blockingIssues.length > 0) {
    const blockerDetail = blockingIssues
      .map(i => `- [BLOCKING] ${i.description}: ${i.revision_instruction}`)
      .join('\n');

    throw ApplicationFailure.create({
      message: `[BUILD-008] QUALITY GATE BLOCKED ${reviewingAgent}\n${review.revision_summary}\n\nBlocking issues:\n${blockerDetail}`,
      type: 'QualityGateBlocked',
      nonRetryable: true,
      details: [{ review, reviewingAgent, ticketId }]
    });
  }
