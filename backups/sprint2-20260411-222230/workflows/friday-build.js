...
export async function FridayBuildWorkflow(jobData) {
  ...
  // ── BUILD-006 + BUILD-008 revision loop ──────────────────────────────────────
  let schemaResult = null;
  let schemaGateResult = null;
  let schemaRevisionCount = 0;
  const MAX_SCHEMA_REVISIONS = 3;

  while (schemaRevisionCount <= MAX_SCHEMA_REVISIONS) {
    try {
      schemaResult = await agentActivities.schemaArchitectActivity({
        ...jobData,
        _revisionCount: schemaRevisionCount,
        _revisionFeedback: schemaRevisionCount > 0 ? schemaGateResult?.revision_package : null
      }, contract);
    } catch(e) {
      schemaResult = { agent_id: 'schema_architect', status: 'error', error: e.message };
    }
    await emitAgent('BUILD-006', 'Schema Architect', schemaResult?.status === 'error' ? 'error' : 'complete');

    schemaGateResult = await reviewActivities.qualityGateActivity(
      { ...jobData, _revisionCount: schemaRevisionCount }, 'BUILD-006', schemaResult
    );
    await emitAgent('BUILD-008', 'Quality Gate [006]', schemaGateResult.needs_revision ? 'revision' : 'complete');

    if (!schemaGateResult.needs_revision || schemaRevisionCount >= MAX_SCHEMA_REVISIONS) {
      if (schemaGateResult.needs_revision && schemaRevisionCount >= MAX_SCHEMA_REVISIONS) {
        console.log('[FRIDAY WF] BUILD-008 schema gate: max revisions reached, continuing with warnings');
        await supabaseLog(jobData.ticket_id || jobData.ticketId, 'build_intelligence', {
          source: 'quality_gate', category: 'max_revisions_reached',
          title: `BUILD-006 reached max revisions (${MAX_SCHEMA_REVISIONS}) -- continuing with warnings`,
    ...
    schemaRevisionCount++;
    console.log(`[FRIDAY WF] BUILD-006 revision ${schemaRevisionCount}/${MAX_SCHEMA_REVISIONS}: ${schemaGateResult.revision_summary?.slice(0,100)}`);
  }
  jobData._qualitySignals = jobData._qualitySignals || {};
  jobData._qualitySignals['BUILD-006'] = schemaGateResult;

  // ── BUILD-002 + BUILD-008 revision loop ──────────────────────────────────────
  let workflowResult = null;
  let workflowGateResult = null;
  let workflowRevisionCount = 0;
  const MAX_WORKFLOW_REVISIONS = 3;

  while (workflowRevisionCount <= MAX_WORKFLOW_REVISIONS) {
    try {
      workflowResult = await agentActivities.workflowBuilderActivity({
        ...jobData,
        _revisionCount: workflowRevisionCount,
        _revisionFeedback: workflowRevisionCount > 0 ? workflowGateResult?.revision_package : null
      }, contract);
    } catch(e) {
      // Fallback to blueprint import if Claude Code agent fails
      try {
        workflowResult = await shortActivities.importBlueprintActivity(jobData, null);
      } catch(e2) {
        workflowResult = { imported: [], manifest: null, error: e.message };
      }
    }
    await emitAgent('BUILD-002', 'Workflow Builder', workflowResult?.error ? 'error' : 'complete');

    workflowGateResult = await reviewActivities.qualityGateActivity(
      { ...jobData, _revisionCount: workflowRevisionCount }, 'BUILD-002', workflowResult
    );
    await emitAgent('BUILD-008', 'Quality Gate [002]', workflowGateResult.needs_revision ? 'revision' : 'complete');

    if (!workflowGateResult.needs_revision || workflowRevisionCount >= MAX_WORKFLOW_REVISIONS) {
      if (workflowGateResult.needs_revision && workflowRevisionCount >= MAX_WORKFLOW_REVISIONS) {
        console.log('[FRIDAY WF] BUILD-008 workflow gate: max revisions reached, continuing with warnings');
        await supabaseLog(jobData.ticket_id || jobData.ticketId, 'build_intelligence', {
          source: 'quality_gate', category: 'max_revisions_reached',
          title: `BUILD-002 reached max revisions (${MAX_WORKFLOW_REVISIONS}) -- continuing with warnings`,
    ...
    workflowRevisionCount++;
    console.log(`[FRIDAY WF] BUILD-002 revision ${workflowRevisionCount}/${MAX_WORKFLOW_REVISIONS}: ${workflowGateResult.revision_summary?.slice(0,100)}`);
  }
  ...
  if (workflowResult?.zero_workflow_warning) {
    try {
      await shortActivities.updateBuildDurationActivity(jobData.supabaseBuildId, {});
    } catch(e) {}
    // Attach warning to contract so QA and downstream agents see it
    contract.zero_workflow_warning = workflowResult.zero_workflow_warning;
  }

  // ── BUILD-004 + BUILD-008 revision loop ──────────────────────────────────────
  let llmResult = null;
  let llmGateResult = null;
  let llmRevisionCount = 0;
  const MAX_LLM_REVISIONS = 3;

  while (llmRevisionCount <= MAX_LLM_REVISIONS) {
    try {
      llmResult = await agentActivities.llmSpecialistActivity({
        ...jobData,
        _revisionCount: llmRevisionCount,
        _revisionFeedback: llmRevisionCount > 0 ? llmGateResult?.revision_package : null
      }, contract, { schema: schemaResult });
    } catch(e) {
      llmResult = { agent_id: 'BUILD-004', status: 'error', error: e.message };
    }
    await emitAgent('BUILD-004', 'LLM Specialist', llmResult?.status === 'error' ? 'error' : 'complete');

    llmGateResult = await reviewActivities.qualityGateActivity(
      { ...jobData, _revisionCount: llmRevisionCount }, 'BUILD-004', llmResult
    );
    await emitAgent('BUILD-008', 'Quality Gate [004]', llmGateResult.needs_revision ? 'revision' : 'complete');

    if (!llmGateResult.needs_revision || llmRevisionCount >= MAX_LLM_REVISIONS) {
      if (llmGateResult.needs_revision && llmRevisionCount >= MAX_LLM_REVISIONS) {
        console.log('[FRIDAY WF] BUILD-008 LLM gate: max revisions reached, continuing with warnings');
        await supabaseLog(jobData.ticket_id || jobData.ticketId, 'build_intelligence', {
          source: 'quality_gate', category: 'max_revisions_reached',
          title: `BUILD-004 reached max revisions (${MAX_LLM_REVISIONS}) -- continuing with warnings`,
    ...
    llmRevisionCount++;
    console.log(`[FRIDAY WF] BUILD-004 revision ${llmRevisionCount}/${MAX_LLM_REVISIONS}: ${llmGateResult.revision_summary?.slice(0,100)}`);
  }
  ...
  // ── BUILD-005 + BUILD-008 revision loop ──────────────────────────────────────
  let platformResult = null;
  let platformGateResult = null;
  let platformRevisionCount = 0;
  const MAX_PLATFORM_REVISIONS = 3;

  while (platformRevisionCount <= MAX_PLATFORM_REVISIONS) {
    try {
      platformResult = await agentActivities.platformBuilderActivity({
        ...jobData,
        _revisionCount: platformRevisionCount,
        _revisionFeedback: platformRevisionCount > 0 ? platformGateResult?.revision_package : null
      }, contract, { schema: schemaResult, workflow: workflowResult });
    } catch(e) {
      platformResult = { agent_id: 'platform_builder', status: 'error', error: e.message };
    }
    await emitAgent('BUILD-005', 'Platform Builder', platformResult?.status === 'error' ? 'error' : 'complete');

    platformGateResult = await reviewActivities.qualityGateActivity(
      { ...jobData, _revisionCount: platformRevisionCount }, 'BUILD-005', platformResult
    );
    await emitAgent('BUILD-008', 'Quality Gate [005]', platformGateResult.needs_revision ? 'revision' : 'complete');

    if (!platformGateResult.needs_revision || platformRevisionCount >= MAX_PLATFORM_REVISIONS) {
      if (platformGateResult.needs_revision && platformRevisionCount >= MAX_PLATFORM_REVISIONS) {
        console.log('[FRIDAY WF] BUILD-008 platform gate: max revisions reached, continuing with warnings');
        await supabaseLog(jobData.ticket_id || jobData.ticketId, 'build_intelligence', {
          source: 'quality_gate', category: 'max_revisions_reached',
          title: `BUILD-005 reached max revisions (${MAX_PLATFORM_REVISIONS}) -- continuing with warnings`,
    ...
    console.log(`[FRIDAY WF] BUILD-005 revision ${platformRevisionCount}/${MAX_PLATFORM_REVISIONS}: ${platformGateResult.revision_summary?.slice(0,100)}`);
  }
  ...
  while (iterationCycle < MAX_ITERATION_CYCLES) {
    ...
    for (const failure of qaTestResult.failures) {
      ...
      if (agent === 'BUILD-006' && schemaResult) {
        // Re-run schema architect with failure context
        try {
      ...
      if (agent === 'BUILD-002' && workflowResult) {
        // Re-run workflow import (rebuild needed upstream)
        try {
          workflowResult = await shortActivities.importBlueprintActivity(jobData, null);
        } catch(e) {
      ...
      if (agent === 'BUILD-005' && platformResult) {
        // Re-run platform builder with failure context
        try {
